...
public class NegocioCoreProveedorMovimientoService {
    ...
    public Mono<List<NegocioCoreProveedorMovimiento>> findAllInformacionCompras(String backendServer, String backendPort, OffsetDateTime desde, OffsetDateTime hasta) {
                ...
                .onStatus(HttpStatusCode::isError, response ->
                        response.bodyToMono(String.class)
                                .flatMap(errorBody -> {
                                    ...
                                    return Mono.error(new NegocioException("Error calling proveedormovimiento service: " + errorBody));
                                })
                )
                .bodyToFlux(NegocioCoreProveedorMovimiento.class)
                .collectList()
                .doOnSuccess(values -> log.debug("Received values: {}", values))
                .doOnError(error -> log.error("Failed to retrieve proveedormovimiento. Reason: {}", error.getMessage()));
    }
