...
public class GeneraComprasService {
    ...
    public void generateFiles(Map<String, String> filenames, OffsetDateTime desde, OffsetDateTime hasta,
            Boolean correccionCompras) throws IOException {
        ...
        for (var negocio : negocioService.findAll()) {
            ...
            for (var proveedorMovimiento : Objects.requireNonNull(negocioCoreProveedorMovimientoService
                    .findAllInformacionCompras(negocio.getBackendServer(), negocio.getBackendPort(), desde, hasta)
                    .block())) {
                ...
                if (!Tool.validaCuit(numeroDocumento)) {
                    outErroresCompras.write("ERROR: Negocio " + negocio.getNombre() + " - Proveedor "
                            + proveedor.getRazonSocial() + " CUIT " + proveedor.getCuit() + " NO Válida. Se sugiere "
                            + Tool.generaCuit(numeroDocumento.substring(2, 10), "M")
                            + " - Cambiado por 20-11111111-2 . . ." + "\r\n");
                    numeroDocumento = "20111111112";
                }
                if (originalNumeroDocumento.equals("20111111112")) {
                    outErroresCompras.write("ERROR: Negocio " + negocio.getNombre() + " - Proveedor "
                            + proveedor.getRazonSocial() + " CUIT " + proveedor.getCuit()
                            + " NO Válida. El CUIT es de TERMALIA - Cambiado por 20-11111111-2 . . ." + "\r\n");
                    numeroDocumento = "20111111112";
                }

                if (proveedorMovimiento.getPrefijo() == 0) {
                    outErroresCompras.write("ERROR: Negocio " + negocio.getNombre() + " - Proveedor "
                            + proveedor.getRazonSocial() + " Comprobante "
                            + proveedorMovimiento.getComprobante().getDescripcion() + " "
                            + proveedorMovimiento.getPrefijo() + "/" + proveedorMovimiento.getNumeroComprobante()
                            + " con Punto de Venta 0 - Cambiado por 1 . . ." + "\r\n");
                    proveedorMovimiento.setPrefijo(1);
                }

                String key = numeroDocumento + "." + proveedorMovimiento.getPrefijo() + "."
                ...
                if (proveedorMovimiento.getMontoIva().abs().compareTo(BigDecimal.ZERO) > 0) {
                    String lineaAlicuotas = String.format("%03d",
                            proveedorMovimiento.getComprobante().getComprobanteAfip().getComprobanteAfipId());
                    ...
                    BigDecimal netoGravado = proveedorMovimiento.getMontoIva().abs().divide(new BigDecimal("0.21"), 2,
                    ...
                    lineaAlicuotas += "0005";
                    lineaAlicuotas += String.format("%015d",
                            proveedorMovimiento.getMontoIva().abs().multiply(new BigDecimal("100")).longValue());
                ...
                if (proveedorMovimiento.getMontoIva105().abs().compareTo(BigDecimal.ZERO) > 0) {
                    ...
                    lineaAlicuotas += String.format("%02d", tipoDocumento);
                    lineaAlicuotas += String.format("%020d", Long.parseLong(numeroDocumento));
                    BigDecimal netoGravado = proveedorMovimiento.getMontoIva105().abs().divide(new BigDecimal("0.105"),
                    ...
                    lineaAlicuotas += "0004";
                    lineaAlicuotas += String.format("%015d",
                            proveedorMovimiento.getMontoIva105().abs().multiply(new BigDecimal("100")).longValue());
                    outComprasAlicuotas.write(lineaAlicuotas + "\r\n");
                    cantidadAlicuotas++;
                }
                ...
                if (proveedorMovimiento.getMontoIva27().abs().compareTo(BigDecimal.ZERO) > 0) {
                    ...
                    lineaAlicuotas += String.format("%02d", tipoDocumento);
                    lineaAlicuotas += String.format("%020d", Long.parseLong(numeroDocumento));
                    BigDecimal netoGravado = proveedorMovimiento.getMontoIva27().abs().divide(new BigDecimal("0.27"), 2,
                            RoundingMode.HALF_UP);
                    lineaAlicuotas += String.format("%015d", netoGravado.multiply(new BigDecimal("100")).longValue());
                    netoComprobante = netoComprobante.add(netoGravado);
                    totalComprobante = totalComprobante.add(netoGravado);
                    lineaAlicuotas += "0006";
                    lineaAlicuotas += String.format("%015d",
                            proveedorMovimiento.getMontoIva27().abs().multiply(new BigDecimal("100")).longValue());
                    outComprasAlicuotas.write(lineaAlicuotas + "\r\n");
                    cantidadAlicuotas++;
                }
                ...
                if (proveedorMovimiento.getMontoIva().abs().compareTo(BigDecimal.ZERO) > 0
                        && proveedorMovimiento.getMontoIva27().abs().compareTo(BigDecimal.ZERO) == 0
                        && proveedorMovimiento.getMontoIva105().abs().compareTo(BigDecimal.ZERO) == 0) {
                ...
                // Ajusta el neto e ivas si es necesario
                ...
                if (diferencia.compareTo(new BigDecimal("0.10")) > 0) {
                    ...
                    var errorDescription = "ERROR: Negocio " + negocio.getNombre() + " - Proveedor "
                            + proveedor.getRazonSocial() + " Comprobante "
                            + proveedorMovimiento.getComprobante().getDescripcion() + " "
                            + proveedorMovimiento.getPrefijo() + "/" + proveedorMovimiento.getNumeroComprobante()
                            + " con CREDITO FISCAL Inconsistente - NETO Informado: "
                            + String.format("%.2f", proveedorMovimiento.getNeto()) + " - NETO Esperado: "
                            + String.format("%.2f", netoComprobante) + " . . ." + "\r\n";
                    log.debug("\n\n" + errorDescription + "\n\n");
                    outErroresCompras.write(errorDescription);
                    var netoAjustado = Tool.ajustarNeto(proveedorMovimiento.getImporte(),
                            proveedorMovimiento.getMontoIva(),
                            proveedorMovimiento.getMontoIva105(),
                            proveedorMovimiento.getMontoIva27(),
                            proveedorMovimiento.getPercepcionIva(),
                            proveedorMovimiento.getPercepcionIngresosBrutos(),
                            proveedorMovimiento.getGastosNoGravados());
                    log.debug("Neto Ajustado -> {}", netoAjustado.jsonify());
                    if (proveedorMovimiento.getNeto().compareTo(netoAjustado.getNeto()) != 0 ||
                            proveedorMovimiento.getMontoIva().compareTo(netoAjustado.getMontoIva21()) != 0 ||
                            proveedorMovimiento.getMontoIva105().compareTo(netoAjustado.getMontoIva105()) != 0 ||
                            proveedorMovimiento.getMontoIva27().compareTo(netoAjustado.getMontoIva27()) != 0) {
                        log.debug("\n\nDebe ajustarse el registro del comprobante\n\n");
                        if (correccionCompras) {
                            proveedorMovimiento = negocioCoreProveedorMovimientoService
                                    .adjustNetoAndIvas(negocio.getBackendServer(), negocio.getBackendPort(),
                                            proveedorMovimiento.getProveedorMovimientoId(), netoAjustado)
                                    .block();
                        }
                    }
                }

                assert proveedorMovimiento != null;
                BigDecimal importe = proveedorMovimiento.getImporte();
                ...
                if (totalComprobante.subtract(proveedorMovimiento.getImporte().abs()).abs()
                ...
                lineaComprobante.append(String.format("%03d",
                        proveedorMovimiento.getComprobante().getComprobanteAfip().getComprobanteAfipId()));
                ...
                lineaComprobante
                        .append(String.format("%015d", importe.abs().multiply(new BigDecimal("100")).longValue()));
                lineaComprobante.append(String.format("%015d",
                        proveedorMovimiento.getMontoFacturadoC().abs()
                                .add(proveedorMovimiento.getGastosNoGravados().abs()).multiply(new BigDecimal("100"))
                                .longValue()));
                lineaComprobante.append("0".repeat(15));
                lineaComprobante.append(String.format("%015d",
                        proveedorMovimiento.getPercepcionIva().abs().multiply(new BigDecimal("100")).longValue()));
                lineaComprobante.append("0".repeat(15));
                lineaComprobante.append(String.format("%015d", proveedorMovimiento.getPercepcionIngresosBrutos().abs()
                ...
                BigDecimal totalIva = proveedorMovimiento.getMontoIva().add(proveedorMovimiento.getMontoIva105())
                        .add(proveedorMovimiento.getMontoIva27());
                lineaComprobante
                        .append(String.format("%015d", totalIva.abs().multiply(new BigDecimal("100")).longValue()));
                lineaComprobante.append("0".repeat(15));
                ...
                if (diferencia.compareTo(new BigDecimal("0.50")) > 0) {
                    ...
                    var errorDescription = "ERROR: Negocio " + negocio.getNombre() + " - Proveedor "
                            ...
                            + proveedorMovimiento.getComprobante().getDescripcion() + " "
                            + proveedorMovimiento.getPrefijo() + "/" + proveedorMovimiento.getNumeroComprobante()
                            + " con TOTAL Inconsistente - TOTAL Informado: "
                            + String.format("%.2f", proveedorMovimiento.getImporte()) + " - TOTAL Esperado: "
                            + String.format("%.2f", totalComprobante) + " . . ." + "\r\n";
                    log.debug("\n\n" + errorDescription + "\n\n");
                    outErroresCompras.write(errorDescription);
                }

                neto = neto.add(proveedorMovimiento.getNeto());
                exento = exento.add(proveedorMovimiento.getMontoFacturadoC())
                        .add(proveedorMovimiento.getGastosNoGravados());
